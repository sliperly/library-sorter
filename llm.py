...
ПРАВИЛА:
1. identified=true, если знаешь название и автора (пусть даже приблизительно) —
   это НЕ зависит от того, насколько уверенно определяется тема/жанр/раздел.
   Даже если аннотация путаная, а тема неочевидна (например, короткий
   рассказ без чёткого жанра) — если название и автор ясны, всё равно
   identified=true. Неуверенность в теме выражай через низкий confidence
   и, если нужно, category="_Unprocessed", а НЕ через identified=false.
   identified=false — только если название/автора вообще невозможно
   определить, независимо от темы
